...
void PrintBoard(const Cublet& cublet) {
    for (int r = 0; r < 7; ++r) {
...
void readboard(ColorBlock& colorBlock) {
    for (int r = 0; r < 6; ++r) {
...
Cublet RotFront(const Cublet& cublet) {
...
std::vector<int> Solve(const Cublet& srcCublet, const Cublet& destCublet) {
    ...
    while (!moveQueue.empty()) {
        ...
        Cublet rotatedFront = RotFront(currentCublet);
...
int main() {
    ...
    PrintBoard(sourceCublet);
    printf("\n");
    printf("End cublet configuration is:");
    PrintBoard(destinationCublet);
    printf("\n");
